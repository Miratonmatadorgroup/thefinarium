...
const BuildAndLaunch = () => {
    ...
    return (
        <div className='w-full'>
            <div className="w-11/12 flex-col lg:flex-row mx-auto flex items-center justify-between gap-16">
                <div className="w-full lg:w-1/2">
                    <div className="w-full flex items-start flex-col gap-5">
                        <RoundOutlineButton text={`Build & Launch`} />
                        <div className={`${lead} font-bold text-[39px] `}>Build & Launch Faster with Our Complete Embedded Finance Platform</div>
                        <div className={`text-[16px] w-10/12 ${lead}`}>We make it effortless to integrate banking, payments, and lending into your business—without the complexity of compliance, licensing, or infrastructure.</div>
                        <div className="grid w-full grid-cols-1 md:grid-cols-2 gap-5">
                            {options.map((item, i) => (
                                <div className="flex items-center gap-2 w-full p-2 border rounded-md border-[#cc5802]">
                                    <img src={item.img} alt={`${item.title} image`} />
                                    <div className=""><span className={`${lead} font-semibold text-[16px]`}>{item.title}</span> <span>{item.desc}</span></div>
                                </div>
                            ))}
                        </div>
